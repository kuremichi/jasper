...
class JasperEngine {
    ...
    extractCompositeDependencyTasks(accumulator, compositeDependency, context) {
        const compositeDependencyResponse = {
        ...
        const whenSubscription = rxjs_1.defer(() => {
            return compositeDependency.when ? this.processExpression(compositeDependency.when, context).pipe(operators_1.switchMap(r => {
                return rxjs_1.of(lodash_1.default.get(r, '[0]', false));
            })) : rxjs_1.of(true);
        }).subscribe({
            next: (when) => {
                if (when) {
                    accumulator[compositeDependency.name] = rxjs_1.of(true).pipe(operators_1.switchMap(() => {
                        compositeDependencyResponse.startDateTime = moment_1.default.utc().toDate();
                        ...
                        return rxjs_1.of(compositeDependencyResponse);
                    }));
                }
                else {
                    ...
                    accumulator[`${compositeDependency.name}`] = rxjs_1.of(compositeDependencyResponse);
                }
            },
            error: (err) => {
     ...
     * @param context
     */
    collectDependencyTasks(compositeDependency, context) {
        const tasks = lodash_1.default.reduce(compositeDependency.rules, (accumulator, rule) => {
            if (rule_config_1.isSimpleDependency(rule)) {
                this.extractSimpleDependencyTasks(accumulator, rule, context);
            }
            /* istanbul ignore else  */
            else if (rule_config_1.isCompositeDependency(rule)) {
                this.extractCompositeDependencyTasks(accumulator, rule, context);
            }
            return accumulator;
        }, {});
        return tasks;
    ...
    processCompositeDependency(compositeDependency, context) {
        const operator = compositeDependency.operator || rule_config_1.Operator.AND;
        const executionOrder = compositeDependency.executionOrder || rule_config_1.ExecutionOrder.Parallel;
        const response = {
            name: compositeDependency.name,
            hasError: false,
            isSkipped: false,
    ...
    execute(params) {
        ...
        operators_1.switchMap(() => {
            if (rule.dependencies && rule.dependencies.rules && rule.dependencies.rules.length) {
                return this.processCompositeDependency(rule.dependencies, context).pipe(operators_1.tap((dependencyReponse) => {
